        ...
        <ul className="navbar-nav min-width-130">
          <li className="nav-item dropdown">
            <a className="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown"
              ...
              <img className="header-img" onError={(e)=>{e.target.src=defaultImg}} src={this.props.user.details.image}/> {this.props.user.details.name || 'N.A'}
            </a>
  ...
  render() {
    ...
    <nav className='navbar navbar-expand-lg navbar-dark bg-dark'>
      <a className="navbar-brand" href={this.menu.list.home.url}>
        <img className="header-img" src={logo} alt="Ethereum"/>
      </a>
